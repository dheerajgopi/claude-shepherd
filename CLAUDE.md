...
**Automated commits** follow exact format strings: `tdd(<slug>): chore — add <framework> | red | red(<n>) | green` (see contracts module). There is no spec commit — `.shepherd/` is gitignored, so Loop 1 approval only advances the phase. Red and red(n) stage the changed files the test classifier accepts (`tdd_git.changed_files_matching`), not a fixed path list. The bootstrap `chore` commit is the only `tdd(...)` commit that touches source-controlled files outside the classifier (dependency manifest + lockfiles, never tests). Nothing outside the engine may create commits matching `tdd(...)`.
