...
def load_config(repo_root: Path) -> ShepherdConfig:
    """Parse .shepherd/config.yaml into a ShepherdConfig.

    Missing file raises ShepherdError(SHEPHERD_NOT_INITIALIZED). Unknown keys
...
class StateStore:
    ...
    def __init__(self, feature_dir: Path) -> None:
        """Bind the store to `feature_dir` (.shepherd/features/<slug>)."""

        self.feature_dir = feature_dir
        self.path = feature_dir / STATE_FILE

    def load(self) -> FeatureState:
        """Read state.json; ShepherdError(INTERNAL_ERROR) if missing or corrupt."""
...
def list_features(repo_root: Path) -> list[tuple[str, str]]:
    """All (slug, phase) pairs under .shepherd/features, phase 'UNKNOWN' if unreadable."""
...
def resolve_feature(
    ...
    ShepherdError(NO_FEATURE_RESOLVED) listing existing features and phases.
